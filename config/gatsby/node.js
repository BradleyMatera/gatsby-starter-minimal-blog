...
async function resolveProjectHubVersion() {
  const validate = (value) => {
    if (!/^[a-f0-9]{40}$/i.test(value)) {
      throw new Error('ProjectHub version must be a full 40-character commit SHA.');
    }
    return value.toLowerCase();
  };
  if (process.env.GATSBY_PROJECTHUB_VERSION !== undefined) {
    return validate(process.env.GATSBY_PROJECTHUB_VERSION);
  }
  ...
  try {
    const deadline = new Promise((_, reject) => {
      timer = setTimeout(() => {
        controller.abort();
        reject(new Error('ProjectHub version resolution timed out.'));
      }, 8000);
    });
    return await Promise.race([
      (async () => {
        ...
        return validate((await res.text()).trim());
      })(),
      deadline
    ]);
  } catch (error) {
    throw new Error('Could not resolve ProjectHub SHA; set GATSBY_PROJECTHUB_VERSION to the verified full release commit SHA.');
  } finally {
    clearTimeout(timer);
  }
}
