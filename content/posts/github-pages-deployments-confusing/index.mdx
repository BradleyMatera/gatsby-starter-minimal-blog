...
{/* visual-break:github-pages-deployments-confusing:1 */}
![Debugging process checkpoint illustration for this section.](/assets/visuals/blog/github-pages-deployments-confusing-break-1.png)
...
## How to deploy with GitHub Actions instead
...
## Deploying a built site with Actions
...
|---------|--------------|-----|
| Workflow passes, site does not update | Pages source still set to “Deploy from a branch” | Go to Settings → Pages → Source → GitHub Actions |
| `Error: Could not retrieve token` | Missing `id-token: write` | Add it under `permissions` |
| `Error: Failed to create deployment` | Missing `pages: write` | Add it under `permissions` |
| 404 on CSS or JS after deploy | `path` points to the wrong folder | Confirm it matches your build output |
| Page stays blank | Base path mismatch | Set `homepage` or equivalent for your build tool |

![Inspecting workflow logs](https://media.giphy.com/media/3o7aD9hC6GgT3KfX2k/giphy.gif)

{/* visual-break:github-pages-deployments-confusing:2 */}
![Verification and access checkpoint illustration for this section.](/assets/visuals/blog/github-pages-deployments-confusing-break-2.png)

## Closing

GitHub Pages is not complicated once you respect the base path and the deployment source. Whether you use a `gh-pages` branch or a GitHub Actions workflow, the same rule applies: your build output must know where it will live, and you must confirm the live URL actually serves the files you expect.

If you take one thing from this post, take the verification step. Open DevTools. Check for 404s. Refresh until the timestamp proves you are looking at the new build. That single habit will save you hours of confusion.
