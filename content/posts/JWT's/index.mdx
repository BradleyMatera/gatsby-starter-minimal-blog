...
- Solved a winter CTF by decoding a chained ladder of **JWT access tokens** that guarded a set of gRPC endpoints.
- Used Libsodium, `grpcurl`, and custom Python scripts to verify signatures, rotate credentials, and capture proxy responses.
- Discovered that an AWS load balancer rejected streaming calls because of **metadata casing** and `application/grpc` content-type mismatches.
