...
description: "A hands-on JWT security lab: generate real tokens, decode them, verify signatures, and deliberately break them to see exactly how trust works."
theme: "Security"
---

JWTs are not "magic auth." A JWT is just a string. (See [RFC 7519](https://datatracker.ietf.org/doc/html/rfc7519) for the official specification.) The only time it becomes useful is when you verify it correctly and treat its claims like untrusted input until that verification succeeds. Most JWT confusion comes from mixing up three different actions that sound similar but are not the same thing: creating a token, decoding a token, and verifying a token.
...
Each part is base64url-encoded. That encoding is not encryption. It is just a compact way to transport bytes as text.
...
{/* visual-break:JWT's:1 */}
![Execution process checkpoint illustration for this section.](/assets/visuals/blog/JWT's-break-1.png)
...
If you do not see those states happen, you did not actually learn JWT behavior yet. You only read about it.

{/* visual-break:JWT's:2 */}
![Delivery workflow checkpoint illustration for this section.](/assets/visuals/blog/JWT's-break-2.png)

## What you need before starting

You need [Node 18+](https://nodejs.org/en/about/previous-releases) and a terminal. No frameworks. No web server. No database. This is intentionally small so you can see what is happening.

## Step 1: Create a clean lab project

Create a folder, initialize Node, and install one dependency ([jsonwebtoken on npm](https://www.npmjs.com/package/jsonwebtoken)).
...
Instead of pasting a giant file all at once, we will build this lab in layers so every moving part is visible.
...
`require("jsonwebtoken")` loads the library that knows how to sign and verify tokens. Nothing here is JWT-specific yet. It is just a Node module import.

`SECRET` is the shared key. With HS256, the same secret signs and verifies. Anyone with this value can mint valid tokens. That is why real projects never hardcode it.
...
Libraries make things feel magical. We are going to remove the magic and decode by hand so you see exactly what is inside.
...
Now that you have decoded manually, let us prove the library does the same thing.
...
If your server expects HS256 but receives a token claiming a different algorithm, verification should reject it. That is why we pass `algorithms: ["HS256"]` to verify.
...
JWTs are not complicated once you separate reading from trusting. Decode is useful for debugging. Verification is the actual security step. If your code accepts claims without verifying, you are not using JWTs correctly — you are just parsing JSON with extra steps.

## Keep reading

- [Containerization and Orchestration: A Small Lab You Can Run](/containerization-and-orchestration/)
- [OBJ Parser Lab: Count Vertices and Faces in Zig](/obj-parser/)
