...
> **Context:** Same Zig OBJ parser I use for learning—no production games rely on it.  
> **AI assist:** ChatGPT/Copilot drafted portions of the code; I rewrote, documented, and tested them before publishing.  
> **Status:** Benchmarks + coverage numbers come from my laptop and Valgrind runs; they’re directional, not enterprise SLAs.
...
## Results
- Parsed 10,000-vertex OBJ models in under 10ms on my M2 Max—roughly five times faster than my JavaScript parser.  
- No leaks detected via Valgrind because every allocation routes through explicit `deinit` paths.  
- Unit tests cover ~85% of the codebase (per `zig test --test-cmd` output), including malformed vertex and face inputs.
