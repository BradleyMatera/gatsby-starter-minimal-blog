...
```html
<script src="https://bradleymatera.github.io/ProjectHub/ProjectHub.js?v=15"></script>
```

That's the whole pitch from the outside. What it took to get there is a lot messier than one script tag suggests. The current version has a vanilla JS frontend, a Node backend on a Google Cloud e2-micro VM ([GCP Always Free tier](https://cloud.google.com/free)), a knowledge base pulled from GitHub, a network of free LLM providers, a response cache, per-tab memory, safety checks, a self-improvement loop, and an analytics dashboard. It also has six test suites and more documentation than I expected.
...
Frontend is GitHub Pages ([GitHub Pages documentation](https://docs.github.com/en/pages)). Backend is a tiny GCP VM. Knowledge is a JSON file in the repo. The important part is that if every AI provider fails, Scout still has a real answer from the knowledge base. The fallback isn't a sorry message. It actually works.

### Architecture layer comparison

| Layer | Technology | Cost | Why this choice |
|---|---|---|---|
| Frontend hosting | GitHub Pages | Free | Serves static JS widget with no build step |
| Backend hosting | GCP e2-micro VM | Free (Always Free tier) | Runs Node.js API with persistent process |
| Knowledge base | JSON file in GitHub repo | Free | Versioned, no database to maintain |
| AI providers | Groq, Cloudflare, GitHub Models, Gemini, Grok | Free tiers | Failover network; no single paid dependency |
| HTTPS | Caddy + Let's Encrypt | Free | Automatic TLS certificate renewal |
| Analytics | Vite + Carbon Charts | Free | Built assets committed to GitHub Pages |

## Why vanilla JavaScript

I didn't want to force anyone to install a React component or deal with npm just to embed a chat widget. Some portfolio sites are just static HTML. Some are WordPress. Some are whatever the person already has. If the widget needs a build step, most people won't use it. Vanilla JavaScript runs in every browser without a bundler ([MDN: JavaScript](https://developer.mozilla.org/en-US/docs/Web/JavaScript)).
...
The production server is in a file called `server-gemini.js`. The name is misleading. It is a leftover from when the backend only used Gemini. Now it supports Groq ([Groq API](https://console.groq.com/docs)), Cloudflare Workers AI ([Cloudflare Workers AI docs](https://developers.cloudflare.com/workers-ai/)), GitHub Models ([GitHub Models docs](https://docs.github.com/en/github-models)), Google Gemini ([Gemini API](https://ai.google.dev/gemini-api/docs)), xAI Grok ([xAI API](https://docs.x.ai/)), and an optional OpenAI-compatible endpoint. The main endpoints are `/`, `/health`, `/api/knowledge-health`, `/api/chat`, and `/api/think`.
...
Because LLMs hallucinate resume facts. They will confidently say you have production experience you don't have. They will smooth over gaps. They will make you sound more senior than you are. For a recruiter assistant, that is dangerous. Research on LLM hallucinations shows that models can fabricate plausible-sounding but incorrect details with high confidence ([Google Research: Hallucinations in LLMs](https://research.google/blog/reducing-hallucinations-in-large-language-models/)).
...
Every message walks through a sequence of checks. Here is the full pipeline in order:

1. **Cache check** — Return immediately if the same question was recently answered.
2. **Load knowledge** — Pull the knowledge base JSON into memory.
3. **Safety checks** — Block prompt injection, secret extraction, XSS, and social engineering.
4. **False-claim checks** — Reject requests that try to inflate titles or experience.
5. **Learned answers** — Check if Think Mode already produced a verified answer for this question.
6. **Grounded answer** — Run through 40+ deterministic handlers for factual questions.
7. **`mustStayGrounded` decision** — Determine whether the question is safe for LLM generation.
8. **LLM provider network** — If open-ended, route to free providers with failover.
9. **Validation** — Clean, check length, strip filler, reject unsupported facts.
10. **Shaping and follow-ups** — Format the response and generate suggested next questions.
11. **Weak-answer check** — If the answer was weak, stash it for Think Mode.
...
| Provider | Free tier limits | Strengths | Weaknesses |
|---|---|---|---|
| Groq | Rate-limited requests/day | Extremely fast inference | Strict rate limits; can hit caps quickly |
| Cloudflare Workers AI | Neurons/day quota | Global edge network | Limited model selection |
| GitHub Models | Requests/day per model | Easy token auth with GitHub | Rate limits per model; models rotate |
| Google Gemini | Requests/min and/day | Strong reasoning quality | Free tier quotas can shift |
| xAI Grok | Limited free credits | Good for open-ended questions | Newest provider; least stable |
...
I didn't add a vector database because the knowledge base is small enough to just include the relevant context directly in the prompt. For each generative request, the server builds a prompt with verified facts, behavior rules, the grounded answer, recent session context, the current question, and strict instructions not to use outside facts or exaggerate. This is a lightweight form of Retrieval-Augmented Generation (RAG) that skips vector embeddings entirely ([Retrieval-Augmented Generation — original paper](https://arxiv.org/abs/2005.11401)).
...
A 200 status doesn't mean the answer is good. Every generated response gets cleaned and validated. Here is what the validation layer checks:

- **Preamble removal**: Strip introductory filler like "Sure!" or "Great question."
- **Corporate filler stripping**: Remove phrases like "leverages cutting-edge technologies."
- **Minimum length check**: Reject answers that are too short to be useful.
- **Out-of-scope rejection**: Block responses that wander into topics Scout shouldn't cover.
- **Unsupported number flagging**: Catch statistics or figures not present in the knowledge base.
- **False claim detection**: Block any response that inflates titles or experience.
- **Context-bound fact checking**: Reject answers that introduce facts not in the supplied context.
...
The backend uses Express ([Express.js documentation](https://expressjs.com/)) with the `cors` package and validates the `Origin` header manually. Allowed origins come from the `.env` file. I don't add CORS headers in Caddy ([Caddy documentation](https://caddyserver.com/docs/)) because duplicating them can cause browsers to reject the response. For rejected origins, the server returns the response without CORS permission, so the browser blocks it ([MDN: CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS)).
...
GitHub Pages hosts the frontend. The backend runs on an Always Free eligible GCP VM ([GCP Always Free tier](https://cloud.google.com/free)). Caddy ([Caddy](https://caddyserver.com/)) and Let's Encrypt ([Let's Encrypt](https://letsencrypt.org/)) handle HTTPS. The knowledge base is in GitHub. Runtime stats are small local JSON files. The AI layer uses free provider tiers. There is no hosted database, no paid serverless functions, and no permanent OpenAI or Anthropic dependency.

Here is the full cost breakdown:

- **Frontend hosting**: GitHub Pages — $0
- **Backend VM**: GCP e2-micro (Always Free tier) — $0
- **HTTPS certificates**: Let's Encrypt via Caddy — $0
- **Knowledge base storage**: GitHub repository — $0
- **AI inference**: Free tiers of Groq, Cloudflare, GitHub Models, Gemini, Grok — $0
- **Analytics**: Vite + Carbon Charts, built assets on GitHub Pages — $0
...
- [GCP Always Free tier](https://cloud.google.com/free) — Google Cloud Platform free tier documentation including e2-micro VM eligibility.
- [GitHub Pages documentation](https://docs.github.com/en/pages) — Official guide to hosting static sites on GitHub Pages.
- [GitHub Models documentation](https://docs.github.com/en/github-models) — Free LLM inference through GitHub Models.
- [Groq API documentation](https://console.groq.com/docs) — Groq cloud API for fast LLM inference.
- [Cloudflare Workers AI](https://developers.cloudflare.com/workers-ai/) — Cloudflare's edge AI inference platform.
- [Google Gemini API](https://ai.google.dev/gemini-api/docs) — Official Gemini API documentation.
- [xAI API documentation](https://docs.x.ai/) — Grok LLM API documentation.
- [Express.js documentation](https://expressjs.com/) — Node.js web framework used for the backend.
- [Caddy documentation](https://caddyserver.com/docs/) — Caddy web server with automatic HTTPS.
- [Let's Encrypt](https://letsencrypt.org/) — Free TLS certificate authority.
- [MDN: CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) — Cross-Origin Resource Sharing specification.
- [MDN: JavaScript](https://developer.mozilla.org/en-US/docs/Web/JavaScript) — JavaScript language reference.
- [Vite documentation](https://vitejs.dev/guide/) — Frontend build tool used for the analytics dashboard.
- [Carbon Charts](https://carbondesignsystem.com/data-visualization/charts/) — IBM Carbon Design System data visualization components.
- [Retrieval-Augmented Generation paper (Lewis et al., 2020)](https://arxiv.org/abs/2005.11401) — Original RAG research paper.
- [Google Research: Reducing Hallucinations in LLMs](https://research.google/blog/reducing-hallucinations-in-large-language-models/) — Research on LLM hallucination mitigation.
